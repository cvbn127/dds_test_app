...
public:
  ...
  bool init(int rate, const std::string &topic_name,
    ...
    if (mp_participant == nullptr) {
      ...
      PParam.rtps.setName(
          "Participant_publisher"); // You can put here the name you want
      PParam.domainId = 120;
    ...
    if (eprosima::fastrtps::xmlparser::XMLP_ret::XML_ERROR ==
        eprosima::fastrtps::xmlparser::XMLProfileManager::
            fillPublisherAttributes(profile_name, publisher_att)) {
      std::cerr << "Problem loading profile '" << profile_name << "'"
                << ". Using default values." << std::endl;
      publisher_att.topic.topicKind = eprosima::fastrtps::rtps::NO_KEY;
      publisher_att.historyMemoryPolicy =
          eprosima::fastrtps::rtps::PREALLOCATED_WITH_REALLOC_MEMORY_MODE;
      publisher_att.qos.m_publishMode.kind = eprosima::fastdds::dds::
          PublishModeQosPolicyKind_t::ASYNCHRONOUS_PUBLISH_MODE;
    }
    publisher_att.topic.topicDataType =
        myType.getName(); // This type MUST be registered
    publisher_att.topic.topicName = topic_name;

    mp_publisher = eprosima::fastrtps::Domain::createPublisher(
        mp_participant, publisher_att,

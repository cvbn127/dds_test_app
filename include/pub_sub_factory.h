...
private:
  ...
  template <typename T> class ConcreateBuilder : public Builder {
  public:
    using pb_type = T;
    virtual std::unique_ptr<ITools> build_tools() override {
      return std::make_unique<TemplatedTools<pb_type>>();
    };
    virtual std::unique_ptr<IPublisher> build_publisher() override {
      return std::make_unique<TemplatedPublisher<pb_type>>();
    };
    virtual std::unique_ptr<ISubscriber> build_subscriber() override {
      return std::make_unique<TemplatedSubscriber<pb_type>>();
    };
  };

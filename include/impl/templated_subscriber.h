...
namespace dds_test_app
{
  ...
  public:
    ...
    TemplatedSubscriber(eprosima::fastrtps::Participant *participant = nullptr) : mp_participant(participant), mp_subscriber(nullptr){};
    ...
    bool init(const std::string &topic_name, const std::string &profile_name = "test-app-sub-profile") override
    {
      ...
      if (eprosima::fastrtps::xmlparser::XMLP_ret::XML_ERROR
          == eprosima::fastrtps::xmlparser::XMLProfileManager::fillSubscriberAttributes(profile_name, subscriber_att))
      {
        std::cerr << "Problem loading profile '" << profile_name << "'"
                  << ". Using default values." << std::endl;
      }
      subscriber_att.topic.topicDataType = myType.getName(); // Must be registered before the creation of the
                                                             // subscriber
      subscriber_att.historyMemoryPolicy = eprosima::fastrtps::rtps::PREALLOCATED_WITH_REALLOC_MEMORY_MODE;
      subscriber_att.topic.topicKind     = eprosima::fastrtps::rtps::NO_KEY;
      subscriber_att.topic.topicName     = topic_name;

      mp_subscriber = eprosima::fastrtps::Domain::createSubscriber(mp_participant, subscriber_att,

...
auto PubSubFactory::get_builder(const std::string &message_type_name)
...
PubSubFactory::create_publisher(const std::string &message_type_name) {
...
PubSubFactory::create_subscriber(const std::string &message_type_name) {
  auto builder = get_builder(message_type_name);
  if (!builder) {
    return {nullptr};
  }
...
std::unique_ptr<ITools>
PubSubFactory::create_tools(const std::string &message_type_name) {
  auto builder = get_builder(message_type_name);
  if (!builder) {
    return {nullptr};
  }

...
/*!
 * @file RTPRouteTaskPublisher.cpp
...
bool RTPRouteTaskPublisher::init(int rate, const std::string &publisher_topic_name, const std::string &profile_name)
{
    m_rate = rate;
...
void RTPRouteTaskPublisher::PubListener::onPublicationMatched(Publisher* pub,MatchingInfo& info)
{
    (void)pub;
...
void RTPRouteTaskPublisher::run()
{
    ...
    std::vector<rtp_msgs::msg::PathPointWithMetadata> data(1);
    rtp_msgs::msg::PathPointWithMetadata one_point;
    one_point.pose().position().x() = 42;

    one_point.metadata().max_deviation() = -4.2;
    ...
    do
    {
        latest_message.header().stamp().sec() = std::chrono::system_clock::now().time_since_epoch().count();

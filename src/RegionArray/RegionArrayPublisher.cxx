...
RegionArrayPublisher::RegionArrayPublisher(eprosima::fastrtps::Participant *participant) : mp_participant(participant), mp_publisher(nullptr) {}

RegionArrayPublisher::~RegionArrayPublisher() 
{	
...
bool RegionArrayPublisher::init(int rate, const std::string &publisher_topic_name, const std::string &profile_name)
{
    m_rate = rate;
...
void RegionArrayPublisher::PubListener::onPublicationMatched(Publisher* pub,MatchingInfo& info)
{
    (void)pub;
...
void RegionArrayPublisher::publish(const gis_rtk_msgs::msg::RegionArray &msg)
{
    std::cout << "publish" << std::endl;
    mp_publisher->write(const_cast<gis_rtk_msgs::msg::RegionArray *>(&msg));
}

void RegionArrayPublisher::update_message(const gis_rtk_msgs::msg::RegionArray &msg)
{
    latest_message = msg;
}

void RegionArrayPublisher::run()
{
    ...
    std::vector<gis_rtk_msgs::msg::Region> data(2);
    latest_message.regions(data);
    ...
    latest_message.regions().at(0).polygon().points(polygon_points);
    latest_message.regions().at(0).label() = 23;
    latest_message.regions().at(1).polygon().points(polygon_points);
    // Publication code

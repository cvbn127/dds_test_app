...
/*!
 * @file ObjectsArraySubscriber.cpp
...
std::function<void(const gis_rtk_msgs::msg::ObjectsArray&)> ObjectsArraySubscriber::callback = [](auto msg){};
static std::chrono::steady_clock::time_point last_received_timepoint = std::chrono::steady_clock::now();

ObjectsArraySubscriber::ObjectsArraySubscriber(eprosima::fastrtps::Participant *participant) : mp_participant(participant), mp_subscriber(nullptr) {
}

ObjectsArraySubscriber::~ObjectsArraySubscriber() 
{
...
void ObjectsArraySubscriber::set_callback(std::function<void(const gis_rtk_msgs::msg::ObjectsArray&)> cb)
{
    callback = cb;
}

bool ObjectsArraySubscriber::init(const std::string &topic_name, const std::string &profile_name)
{
...
void ObjectsArraySubscriber::SubListener::onSubscriptionMatched(Subscriber* sub,MatchingInfo& info)
{
    (void)sub;
...
void ObjectsArraySubscriber::SubListener::onNewDataMessage(Subscriber* sub)
{
    // Take data
    gis_rtk_msgs::msg::ObjectsArray st;
    ...
    if(sub->takeNextData(&st, &m_info))
    {
        if(m_info.sampleKind == ALIVE)
        {
            ...
            std::cout << "st.objects size " << st.objects().size() << std::endl;
            if (st.objects().size() > 0)
            {
                std::cout << "st.objects[0].polygon.points size " << st.objects().at(0).polygon().points().size() << std::endl;
            }
...
void ObjectsArraySubscriber::run()
{
